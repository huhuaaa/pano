...
% Default output size
if nargin == 1
    % 未指定输出尺寸，那么设定默认的输出图片尺寸；n为图片宽度，output_size为输出宽高
    n = size(im, 2);
...
% 左上角点,以单位1位标准。z方向为1，并以y轴为准，那么x轴坐标为(output_width/output_height)倍。视角为vfov那么坐标如下
...
for idx = 1 : size(views,1)
    ...
    % views(x,y)获取举证x行，y列的值
    yaw = views(idx,1); % 导航角
    pitch = views(idx,2); % 俯仰角
    roll = views(idx,3); % 翻转角
